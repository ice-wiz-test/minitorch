...
def test_other(a: float, b: float) -> None:
    ...
    * neg is an involution and relu is idempotent
    * relu(a) == max(a, 0) and max is commutative
    * inv is its own inverse away from zero
    """
    assert lt(a, b) + lt(b, a) + eq(a, b) == 1.0
    assert neg(neg(a)) == a
    assert relu(relu(a)) == relu(a)
    assert relu(a) == max(a, 0.0)
